...
Story lettering handoff, Design production-input guidance and the [lettering/print pilot](lettering-and-print-workflow.md) are now documented. Human Park provides prepared project-local folders and blank templates; it has not yet supplied a tested native layout/import/export implementation. The previous comic is now received and an initial geometry/metadata intake and provider-research draft exist. Next evidence needed: resolve native/PDF geometry differences, verify links/fonts and exact product configuration, then run a representative spread and dense-text probe, synchronized correction test, native import and actual PDF checks. Promote proven reusable rules into the owning Production framework afterward.
